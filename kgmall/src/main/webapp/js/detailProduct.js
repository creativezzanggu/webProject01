            ...
            success: function(data) {
                ...
                if (data.is_secret == true) {
                    ...
                    if (data.write_auth == true) {
                        aHtml.push('<a href="/board/product/modify.html?board_act=edit&no='+data.no+'&board_no=6&link_product_no='+iProductNo+'" class="df-btn medium normal">게시글 수정</a>');
                    }
                    aHtml.push('</span></p>');
                    aHtml.push('</div>');
                    ...
                    if (data.comment != undefined && data.comment.length != undefined) {
                        ...
                        for (var i=0; data.comment.length > i; i++) {
                            ...
                            if (data.comment[i]['comment_reply_css'] == undefined) {
                                aHtml.push('<li>');
                                aHtml.push('<span class="comment-by">Comment by</span><strong class="name">'+data.comment[i]['member_icon']+' '+data.comment[i]['comment_name']+'</strong>');
                                aHtml.push('<span class="date">'+data.comment[i]['comment_write_date']+'</span>');
                                aHtml.push('<span class="grade '+data.use_point+'"><img src="/web/upload/dfloor_base/web/icon/ico_point_star'+data.comment[i]['comment_point_count']+'.png" alt="'+data.comment[i]['comment_point_count']+'점" /></span>');
                                if (data.comment[i]['comment_reply_display'] == true) {
                                    aHtml.push('<span class="button">'+'<a href="#none" onclick="'+data.comment[i]['action_comment_reply']+'" class="df-btn light small"><i class="fa fa-comments" aria-hidden="true"></i>댓글</a>'+'</span>');
                                }
                    ...
                    if (data.comment_write != undefined) {
                        ...
                        aHtml.push('</span>');
                        aHtml.push(''+data.comment_write['comment']+'<a href="#none" onclick="'+data.comment_write['action_comment_insert']+'" class="df-btn medium detail-cmt-submit">등록</a></p>');
                        ...
                        aHtml.push('<p class="captcha '+data.comment_write['use_captcha']+'">'+data.comment_write['captcha_image']+data.comment_write['captcha_refresh']+data.comment_write['captcha']+'왼쪽의 문자를 공백없이 입력하세요.(대소문자구분)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_reply != undefined) {
                        ...
                        aHtml.push('</span>');
                        aHtml.push(''+data.comment_reply['comment']+'<a href="#none" onclick="'+data.comment_reply['action_comment_insert']+'" class="df-btn medium detail-cmt-submit">등록</a></p>');
                        ...
                        aHtml.push('<p class="captcha '+data.comment_reply['use_captcha']+'">'+data.comment_reply['captcha_image']+data.comment_reply['captcha_refresh']+data.comment_reply['captcha']+'왼쪽의 문자를 공백없이 입력하세요.(대소문자구분)</p>');
                        aHtml.push('</div>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                    ...
                    if (data.comment_secret != undefined) {
                        ...
                        aHtml.push(' <a href="#none" onclick="'+data.comment_secret['action_secret_submit']+'" class="df-btn light small ml-3">확인</a>');
                        aHtml.push(' <a href="#none" onclick="'+data.comment_secret['action_secret_cancel']+'" class="df-btn light small ml-2">취소</a></p>');
                        aHtml.push('</div>');
                        aHtml.push('</form>');
                    }
                }
...
window.$DF = jQuery.noConflict(true);
...
//디자인플로어 setup.js 설정 변수
var setSize = 340;
var DF = {};
DF['ins-banking-li'] = {};
DF['ins-banking-p'] = {};
DF['set-listmain-type'] = {};
DF['set-listmain-desc'] = {};
DF['set-listmain-size'] = {};
DF['set-listmain-grid'] = {};
DF['set-listmain-rows'] = {};
...
/*====================================================================================================
	...
	- 본 파일에서 쇼핑몰 기능의 사용여부 및 표시문구를 쉽게 설정할 수 있습니다.
	- 일부 기능을 사용하지 않는다 하더라도 본 파일에 존재하는 항목은 절대 삭제하지마세요.
	- 수정후 [저장] 을통해 본 파일을 저장해주셔야 적용됩니다.


	■■■ [중요] 수정시 필수 참고사항 ■■■

	- 설정값은 DF[''] = "입력값"; 형식으로, 쌍따옴표("") 안의 내용만(소문자로 입력) 수정하세요.
    - 항목값 수정시 쌍따옴표("") 내용안에 다시 쌍따옴표(")를 입력하시면 오류가 발생합니다.
	- 쌍따옴표안 내용을 작성하실때 엔터키를 입력하지마세요. 사이트 전체에 문제가 발생합니다.


	■■■ [복구] 문제 발생시 해결방법 ■■■

	- 본 파일을 잘못 수정해 사이트에 문제가 생긴경우 아래 2가지 방법으로 복구할 수 있습니다.
	- 방법1: setup_backup.js(원본파일) 내용 복사후 붙여넣기 하세요.(초기 샘플상태로 돌아갑니다.)
	- 방법2: 디자인 백업.복구란에서 최근자로 백업된 디자인을 복구하세요. (백업 시점으로 돌아갑니다.)
...
/****************************************************************************************************
	...
	- 로그인전 회원가입버튼 주위에 보여지는 말풍선아이콘입니다.
	- 가입시 증정할 적립금 또는 기타문구를 짧게 입력하여 활용하실 수 있습니다.
	- 아래를 참고하여 설정해주세요.
...
/****************************************************************************************************
	...
	- 사이트하단에 보여질 고객센터 운영시간을 수정해주세요.
	- 내용이 긴경우 아래 작성예와 같이 줄바꿈 태그 <br> 을 이용하여 작성하세요.

	[작성예]
	평일 오전 10시 ~ 오후 6시 <br> 토/일/공휴일 휴무

****************************************************************************************************/

//고객센터 운영시간을 입력해주세요.
DF['rep-cstime-txt'] = "평일 오전 10:00 ~ 오후 6:00 / 점심시간 오후 12:00 ~ 오후 1:00 <br> 토 / 일 / 공휴일 휴무";
...
/****************************************************************************************************
	...
	- 사이트하단에 표시되는 교환/반품지주소 정보를 입력하세요.
	- 사용하지 않을경우 내용을 절대 삭제하지마시고 반품지주소외 대체 문구로 활용하세요.
...
//첫번째줄 표시문구
DF['rep-returnex-txt1'] = "이곳에 반품지 주소를 입력해주세요";

//두번째줄 표시문구
DF['rep-returnex-txt2'] = "자세한 교환·반품절차 안내는 문의란 및 공지사항을 참고해주세요";
...
/****************************************************************************************************
	...
	- 쇼핑몰 하단에 표시되는 계좌번호를 아래의 항목에서 수정해주세요.
	- 항목 뒷부분의 쌍따옴표("") 안에 표시될 계좌번호 및 예금주명을 입력하시면 됩니다.
	- 계좌번호를 추가할 경우 DF['ins-banking-li'][다음숫자] = ""; 소스를 항목 마지막줄에 추가하세요.
	- 추가된 소스에서 [다음숫자] 부분을 다음숫자로(1234순) 입력해주세요.

****************************************************************************************************/

//입금계좌번호(사용하시는 계좌번호 및 예금주로 변경해주세요)
DF['ins-banking-li'][1] = "국민 000-000-0000";
DF['ins-banking-li'][2] = "예금주: 디자인플로어";
...
/****************************************************************************************************
	...
	- 동영상을 넣는 방법은 구매하신 디자인매뉴얼을 확인해주세요.
	- 아래 각 항목의 입력값을 참고해 뒷부분 쌍따옴표("")안에 원하는 입력값을 입력해주세요.
...
//슬라이드배너 대기시간 (이미지가 멈춰있는 시간을 입력하세요.)
//입력예: 100 = 0.1초, 1000 = 1초
DF['set-mainslider-pause'] = "4500";

//슬라이드배너 효과시간 (이미지가 전환되는 효과시간을 입력하세요.)
//입력예: 100 = 0.1초, 1000 = 1초
...
/****************************************************************************************************
	...
	6.메인화면 첫번째 상품분류 슬라이드·리스트타입 설정

	- 기능 설정값
	  slide: 메인화면 첫 상품분류가 슬라이드형태로 보여집니다.
	  list:  메인화면 첫 상품분류가 일반목록형태로 보여집니다.
	  [중요] list로 설정시 뷰타입·가로·세로표시 설정이 적용되지 않으며 기본설정을 따라갑니다.

	- 뷰타입 설정값
	  view: 상품명·가격등의 정보가 기본형태로 보여집니다.
	  fade: 상품명·가격등의 정보가 상품이미지 위에 페이드로 보여집니다.

	- 가로표시개수 설정값
	  입력된 숫자만큼 가로 표시개수가 변경됩니다.

	- 이미지 가로 최대사이즈
	  표시될 이미지의 가로사이즈입니다. 총 표시될 사이즈가 넓이를 초과할 경우에는 해당값은 무시됩니다.

	- 세로표시개수 설정값
	  입력된 숫자만큼 세로 표시줄이 변경됩니다.

****************************************************************************************************/

//기능설정: slide 또는 list 값을 입력하세요.
DF['set-listmain-type'][1] = "slide";

//뷰타입설정: view 또는 fade 값을 입력하세요.
DF['set-listmain-desc'][1] = "fade";

//이미지 가로사이즈를 입력하세요. (상품목록 사이즈와 동일사이즈 권장)
//본 설정은 기능설정이 slide 일 때만 적용됩니다.
DF['set-listmain-size'][1] = "400";

//가로표시 개수: 3~6 사이 값을 입력하세요.
//본 설정은 기능설정이 slide 일 때만 적용됩니다.
DF['set-listmain-grid'][1] = "3";

//세로표시 개수: 1~6 사이 값을 입력하세요.
//본 설정은 기능설정이 slide 일 때만 적용됩니다.
DF['set-listmain-rows'][1] = "2";
...
/****************************************************************************************************
	...
	7.상품목록이미지 마우스 롤오버기능 사용여부

    - 상품목록 이미지에 마우스를 올렸을 때 다른이미지가 보여지는 기능입니다.
    - 목록이미지가 기본으로 보여지고, 마우스를 올렸을 때 작은목록이미지가 보여집니다.
	- 아래를 참고하여 설정해주세요.

****************************************************************************************************/

//상품목록 이미지 롤오버 기능 사용여부
DF['set-prdrollover'] = "off";
...
//메인 공지사항 표시여부
//메인 공지사항입니다. 표시를 끄면 추가배너로 사용할 수 있습니다.
DF['use-latest'] = "on";


//메인 상품분류 네비게이션 표시여부
//메인에서 스크롤에 따라 상품분류 위치와 이동버튼이 노출되는 기능입니다.
DF['set-movelist'] = "on";


//상품목록 새창보기 아이콘 표시여부
//상품목록의 버튼 클릭시 상품을 새창으로 볼 수 있는 기능입니다.
DF['use-targetblank'] = "on";


//상품 상세페이지 좌우정렬 (left 또는 center 값입력)
//상품 상세페이지의 정렬값을 변경할 수 있습니다. 작성시 강제로 정렬한 경우에는 작동하지 않습니다.
DF['set-detailarea'] = "center";


//상품 상세페이지 하단 구매가이드 표시여부
//상세페이지 하단에 공통으로 표시되는 결제·배송·반품 가이드입니다.
DF['use-guide'] = "on";


//상품 상세페이지 하단 후기게시판 표시여부
DF['use-review'] = "on";


//상품 상세페이지 하단 문의게시판 표시여부
DF['use-qna'] = "on";
...
//상품분류 전체보기 가로 표시개수 (4~8까지 입력가능)
DF['set-allmenu-cols'] = "5";


//상품 상세페이지 스크롤시 구매창 표시여부
DF['set-detail-optionfix'] = "on";
...
/****************************************************************************************************
	...
	9.상세페이지 관련상품 리스트·갤러리타입 설정

	- 기능 설정값
	  list: 관련상품이 리스트로 보여지며 함께구매·장바구니담기 기능이 활성화됩니다.
	  gallery: 관련상품이 갤러리로 보여지며 관련상품이 5개이상일떄 슬라이드로 표시됩니다.
	  all: 리스트타입·갤러리타입 두가지방식이 함께 보여집니다.

****************************************************************************************************/

//관련상품 타입 (list 또는 gallery 또는 all 값입력)
DF['set-relation'] = "all";
...
//카카오톡 버튼 상세페이지 표기문구
DF['rep-yellowid-txt'] = "@디자인플로어";

//카카오톡 링크주소를 변경해주세요.
DF['rep-yellowid-url'] = "https://pf.kakao.com/_ZizJu";
...
DF['use-facebook'] = "on";
//페이스북 주소
DF['put-facebook-href'] = "https://facebook.com/";
...
DF['use-instagram'] = "on";
//인스타그램 주소
DF['put-instagram-href'] = "https://instagram.com/";
...
//즐겨찾기·바로가기 아이콘 표시여부
//디자인관리 > 디자인기능설정 > 쇼핑몰알리미 > 즐겨찾기·바로가기에서 설정하셔야 됩니다.
...
/****************************************************************************************************
	...
	12.메인상품 분류별 배너 위치설정하기

	설명: 메인상품 분류별 배너는 메인상품분류의 사이에 배너를 순차적으로 여러개 등록할 수 있는 기능입니다.

	[기능 입력값]
	before: 등록한 여러개의 배너가 각 메인상품분류의 위에 순차적으로 표시됩니다.
	after: 등록한 여러개의 배너가 각 메인상품분류의 아래에 순차적으로 표시됩니다.

****************************************************************************************************/

//메인분류별 배너 시작점 수정하기 ( before 또는 after 입력 )
...
/****************************************************************************************************
    ...
    - 할인율 기능은 소비자가,판매가,할인가 중 높은금액과 낮은금액을 계산해 퍼센트로 표시됩니다.
	- 상품등록시 소비자가 혹은 할인판매가 둘중 하나가 표시되어야 할인율 기능이 작동합니다.

	[입력값 설명]
	on : 상품목록에 마우스를 올리면 할인율을 표시합니다.
	off: 할인율 표시기능을 사용하지 않습니다.
	fix: 상품목록에 항상 할인율을 표시합니다.

****************************************************************************************************/

//할인율 기능 표시여부 (on 또는 off 또는 fix 를 입력해주세요)
DF['set-discountrate'] = "on";
...
/****************************************************************************************************
    ...
    - 기간할인을 진행중인 상품에 타임세일 카운트를 표시하는 기능입니다.
	- 관리자모드 > 프로모션 > 고객혜택관리 > 혜택등록 에서 기간할인을 진행해주셔야 적용됩니다.


	[분류번호 입력값 설명예제]
	다수의 분류에 적용 할때는 콤마(,)로 구분하여 원하는 분류에만 타임세일을 적용할 수 있습니다.
	메인상품 분류번호는 m1, m2, m3 으로 구분합니다. 예) 메인 첫번째 분류는 m1 이며, 두번째 분류는 m2 입니다.
	일반상품 분류번호는 분류 url(인터넷 주소창) 가장뒤에 표시되는 숫자로 구분합니다.
	아래를 예제를 참고해 분류번호를 입력해주세요.

	- 분류번호를 "m1" 으로 입력하면 메인화면 첫번째 상품분류에 타임세일이 표시됩니다.
	- 분류번호룰 "m1,m2" 으로 입력하면 메인화면 첫번째와 두번째 상품분류에 타임세일이 표시됩니다.
	- 분류번호를 "40" 으로 입력한 경우 40번 상품분류에 타임세일이 표시됩니다.
	- 분류번호를 "40,60" 으로 입력한 경우 40번 과 60번 상품분류에 타임세일이 표시됩니다.
	- 분류번호를 "m1,40" 으로 입력한 경우 메인화면 첫번째와 40번 상품분류에 타임세일이 표시됩니다.
	- 분류번호를 "all"로 입력한 경우 모든 상품분류에서 타임세일이 표시됩니다.

****************************************************************************************************/

//적용상품 분류번호 (사용하지 않을때는 off 를 입력하세요)
DF['set-timesale-cate'] = "off";

//타임세일 종료 메세지 표시여부 (on 또는 off 를 입력하세요.)
...
/**********************************************************************************************************
	...
	- 아래에 표시된 항목의 설정에 따라 스마트리뷰의 다양한 기능을 설정할 수 있습니다.
	- 아래에 설명된 항목의 입력값을 참고하여 설정해주시기 바랍니다.

**********************************************************************************************************/

//메인하단 리뷰 표시여부
//입력값: on 또는 off 를 입력하세요
DF['set-review-mainuse'] = "on";

//메인하단 타입보기 버튼 표시여부
//입력값: on 또는 off 를 입력하세요
DF['set-review-maintypebtn'] = "on";
...
//상세페이지 타입보기 버튼 표시여부
//입력값: on 또는 off 를 입력하세요
DF['set-review-detailtypebtn'] = "on";
...
//상세페이지 리뷰 가로개수 설정하기
//목록 타입이 갤러리(gallery)일 때만 반영됩니다.
DF['set-review-detailgallerycols'] = "4";


//리뷰게시판 타입보기 버튼 표시여부
//입력값: on 또는 off 를 입력하세요
DF['set-review-listtypebtn'] = "on";
...
//리뷰게시판 가로개수 설정하기
//목록 타입이 갤러리(gallery)일 때만 반영됩니다.
DF['set-review-listgallerycols'] = "4";
...
/**********************************************************************************************************
	...
	- 쇼핑몰 하단의 인스타그램 위젯을 설정해보세요.
	- 아래 각 항목의 입력값을 참고하여 설정해주세요.

**********************************************************************************************************/

//인스타그램 위젯 표시여부
//입력값: on 또는 off 를 입력하세요
DF['set-insta-use'] = "off";

//인스타그램 엑세스토큰 설정
//입력값: 엑세스토큰 값을 입력하세요. 토큰값은 디자인 매뉴얼을 참고해주세요
DF['set-insta-accesstoken'] = "이곳에 엑세스토큰값을 입력하세요";

//인스타그램 이미지 총 표시개수 설정
//입력값: 1 에서 60 까지 입력 가능합니다
DF['set-insta-listcount'] = "20";

//인스타그램 이미지 가로개수 설정
//입력값: 1 에서 20까지 입력 가능합니다
DF['set-insta-grid'] = "10";

//인스타그램 이미지별 간격 설정
//입력값: 1 에서 20 까지 입력 가능합니다
DF['set-insta-gridspace'] = "1";


(function($){
if($(location).attr('host').indexOf('ecudemo31431.cafe24.com')>=0){
	...
	$(document).ready(function(){
		...
		function inputImg(src){
			var imgarea = $('.tit-product, .tit-board').find('.img');
		...
		//상품분류 상단이미지
		if(cate_no==84){
			inputImg('/web/upload/dfloor_base/web/sample/top_img_category_1260.jpg');
		}else if(cate_no==42){
			inputImg('/web/upload/dfloor_base/web/sample/top_img_category_1260.jpg');
		}else if(cate_no==46){
			inputImg('/web/upload/dfloor_base/web/sample/top_img_category_1260.jpg');
			...
			inputImg('/web/upload/dfloor_base/web/sample/top_img_category_timesale_1260.jpg');
		}


		//게시판 상단이미지
		if(board_no==3){
			inputImg('/web/upload/dfloor_base/web/sample/top_img_board_1260.jpg');
		}else if(board_no==4){
			inputImg('/web/upload/dfloor_base/web/sample/top_img_board_1260.jpg');
		}else if(board_no==6){
			inputImg('/web/upload/dfloor_base/web/sample/top_img_board_1260.jpg');
		...
		if($(location).attr('pathname')=='/page/brand.html'){
			inputImg('/web/upload/dfloor_base/web/sample/top_img_brand_1260.jpg');
		}
		...
		//메인 동영상 배너에 영상추가
		$(".sec-vbanner ul > li:first-child").prepend('<iframe width="560" height="315" src="https://www.youtube.com/embed/ElnaLT2US14" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
	...
	**********************************************************************************************************/
	...
	DF['set-insta-use'] = "on";
	//인스타그램 엑세스토큰 설정
	DF['set-insta-accesstoken'] = "3116660631.1677ed0.254f987bd37c44808806aabf8976727b";
	//인스타그램 이미지 총 표시개수 설정
	DF['set-insta-listcount'] = "20";
	//인스타그램 이미지 가로개수 설정
	DF['set-insta-grid'] = "10";
...
// Attr change Check 2017-01-09
var MutationObserver = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver;
$DF.fn.attrchange = function(callback){
	if(MutationObserver){
		var options = {
			subtree: false,
			attributes: true
...
//  썸네일 이미지 엑박일경우 기본값 설정 2016-11-25
$(window).load(function() {
	$("img.thumb,img.ThumbImage,img.BigImage,img.thumbSlide").each(function($i,$item){
		var $img = new Image();
		$img.onerror = function () {
			$item.src="/web/upload/dfloor_base/web/common/nonePrdImage.gif";
		}
		$img.src = this.src;
	});
});
...
$(document).ready(function(){
	...
	// 설정파일 관련(use:출력여부, put:attr 내용삽입, rep:문자열 치환, set:별도) 2016-11-23
	var set_load=function(){$.each(DF,function(b,d){if(a=b.split("-"),s=b.split("-"),s.splice(s.length-1,1),id=s.join("-"),"use"==a[0])if($("#"+b).length>0)var e=$("#"+b);else{if(!($("."+b).length>0))return;var e=$("."+b)}else if($("#"+id).length>0)var e=$("#"+id);else{if(!($("."+id).length>0))return;var e=$("."+id)}if("use"==a[0])e.length>0&&("on"==d?e.show():"off"==d&&e.hide());else if("put"==a[0])e.length>0&&e.attr(a[a.length-1],d);else if("rep"==a[0])e.length>0&&(e.length>1?e.each(function(){$(this).parent().html().indexOf("<!--{"+b+"}-->")>=0?$(this).parent().html($(this).parent().html().replace("<!--{"+b+"}-->",d)):$(this).parent().html().indexOf("{"+b+"}")>=0&&$(this).parent().html($(this).parent().html().replace("{"+b+"}",d))}):e.parent().html().indexOf("<!--{"+b+"}-->")>=0?e.parent().html(e.parent().html().replace("<!--{"+b+"}-->",d)):e.parent().html().indexOf("{"+b+"}")>=0&&e.parent().html(e.parent().html().replace("{"+b+"}",d)));else{if("ins"!=a[0])return!0;e.length>0&&(c=0,$.each(DF[b],function(b,d){d.length>0&&(e.append("<"+a[a.length-1]+' class="'+id+"-"+(c+1)+'">'+d+"</"+a[a.length-1]+">"),c++)}))}})};set_load();


	// 배너 이미지 Attr 제거(width·height·hspace·vspace·title 값 제거) 2016-11-22
	if($('.banner_image').length>0){
		$('.banner_image').removeAttr('width height hspace vspace title');
	}


	//이미지 롤오버 클래스(img_on) 2016-11-22
	$(".img_on").length>0&&$(".img_on").mouseover(function(){var a=$(this).attr("src").split("/"),b=a[a.length-1],c="";for(i=0;i<a.length-1;i++)c=0==i?c+a[i]:c+"/"+a[i];$(this).attr("src",c+"/"+b.replace(/.([a-zA-Z]{3,3})$/,"_on.$1"))}).mouseout(function(){var a=$(this).attr("src").split("/"),b=a[a.length-1],c="";for(i=0;i<a.length-1;i++)c=0==i?c+a[i]:c+"/"+a[i];$(this).attr("src",c+"/"+b.replace(/_on.([a-zA-Z]{3,3})$/,".$1"))});


	// 타임세일 2018-03-20
	if("off"!=DF["set-timesale-cate"]){var cateArray=DF["set-timesale-cate"].replace(/ /gi,"").split(","),cateno=df_getParam(location.search).cate_no;if(null==cateno){var url=window.location.pathname.split("/");"category"==url[1]&&(cateno=url[3])}for(var elArr=[],cnt=0,i=0;i<cateArray.length;i++)1==$("#wrap").hasClass("main")&&cateArray[i].match("m")?(elArr[cnt]=$(".xans-product-listmain").eq(parseInt(cateArray[i].replace("m",""))-1),cnt++):cateno&&cateArray[i]==cateno&&(elArr[cnt]=$(".xans-product-listnormal, .xans-product-listrecommend"),cnt++);var elCnt=elArr.length;if(elCnt>0||"all"==DF["set-timesale-cate"]){var el,setCounter;for(i=0;i<elCnt;i++)el=0==i?elArr[i]:el.add(elArr[i]);function timesale(){if("all"==DF["set-timesale-cate"])var e=$(".timesale");else e=el.find(".timesale");var t=e.filter(':not([df-data-timesaleS=""])'),a=t.length,s=e.filter('[df-data-timesaleS=""]'),n=s.length;if(a>0){if(null==document.lastModified){var r;var i=new Date(function(){try{r=new XMLHttpRequest}catch(e){try{r=new ActiveXObject("Msxml2.XMLHTTP")}catch(e){try{r=new ActiveXObject("Microsoft.XMLHTTP")}catch(e){console.log("AJAX not supported")}}}return r.open("HEAD",window.location.href.toString(),!1),r.setRequestHeader("Content-Type","text/html"),r.send(""),r.getResponseHeader("Date")}())}else i=new Date(document.lastModified);function l(e){return e<10?"0"+e:e}function o(e){return'<span><span class="t1"></span><span class="n">'+Math.floor(e/864e5)+'</span><span class="t2"></span><span class="n">'+l(Math.floor(e%864e5/36e5))+'</span><span class="t3"></span><span class="n">'+l(Math.floor(e%36e5/6e4))+'</span><span class="t4"></span><span class="n">'+l(Math.floor(e%6e4/1e3))+'</span><span class="t5"></span></span>'}function c(){serverTime=i.getTime()+1e3,i=new Date(serverTime),t.each(function(e){var t=$(this),a=(t.attr("df-data-timesaleS")+":00").match(/^(\d+)-(\d+)-(\d+) (\d+)\:(\d+)\:(\d+)$/),s=(t.attr("df-data-timesaleE")+":00").match(/^(\d+)-(\d+)-(\d+) (\d+)\:(\d+)\:(\d+)$/),n=new Date(a[1],a[2]-1,a[3],a[4],a[5],a[6]).getTime(),r=new Date(s[1],s[2]-1,s[3],s[4],s[5],s[6]).getTime();serverTime<n?t.find(".before").show().html(o(n-serverTime)).siblings("").hide():serverTime<=r?(t.find(".ing").show().html(o(r-serverTime)).siblings("").hide(),t.parent(".box").find(".discountrate").addClass("show")):t.find(".after").show().siblings("").hide()})}c(),clearInterval(setCounter),setCounter=setInterval(c,1e3),t.show()}"on"==DF["set-timesale-after"]&&n>0&&s.show().find(".after").show().siblings("").hide()}timesale()}}
	...
	$('#wrap.main').each(function(){
		...
		// 메인진열 출력(BX슬라이더) 2018-02-20
		$.each(DF['set-listmain-type'], function(i, v){
			var cnt = i;
			if(typeof DF['set-listmain-type'][cnt]=='undefined'){
				return false;
			}
			var _this = $('.xans-product-listmain').eq(cnt-1);
			var type = DF['set-listmain-type'][cnt];
			var desc = DF['set-listmain-desc'][cnt];
			var size = DF['set-listmain-size'][cnt];
			var grid = DF['set-listmain-grid'][cnt];
			var rows = DF['set-listmain-rows'][cnt];
			// 상품설명
			if(desc=='view'){
			}else if(desc=='hide'){
				_this.find('.prdList').addClass('df-product-boxhide');
				_this.find('.prdList .description').hide();
			}else if(desc=='fade'){
				_this.find('.prdList').addClass('df-product-boxfade');
			}
			// 상품방식
			if(type=='list'){
				_this.find('.box').css('opacity','1');
			}else if(type=='slide'){
				_this.addClass('df-product-slider');
				_this.children('ul').bxSlider({
					auto:true,
					autoHover:true,
					autoDelay:4000,
					speed:740,
		...
		// 메인 2단 배너 사용여부 관련 2019-04-07
		$(this).find(".sec-banner1").each(function(){$(this).find(".banner_image").length>0?$(this).show():$(this).remove()});


		// 메인 4단 배너 사용여부 관련 2019-04-07
		$(this).find(".sec-banner2").each(function(){$(this).find(".banner_image").length>0?$(this).show():$(this).remove()});


		//메인 동영상 배너 사용여부 2019-03-30
		$(this).find(".sec-vbanner").each(function(){$(this).find(".banner_image").length>0?$(this).show():$(this).remove()});
	...
	$('#header').each(function(){
		...
		// 로고 롤오버 2016-11-22
		$(this).find(".top-logo").each(function(){$(this).find(".logo span img").removeAttr("title"),$(this).find(".logo span").eq(1).length&&$(this).find(".logo").mouseenter(function(){$(this).find("span").eq(0).animate({opacity:0},50),$(this).find("span").eq(1).fadeIn(50)}).mouseleave(function(){$(this).find("span").eq(0).animate({opacity:1},50),$(this).find("span").eq(1).fadeOut(50)})});


		// 상단 영역 픽스 2016-10-17
		if(DF["set-fixchooes"]="gnb,menu","undefined"!=typeof DF["set-fixchooes"]&&DF["set-fixchooes"].replace(/ /g,"").length>0){var dffixed=function(){var a=["gnb","menu"];DF["set-fixchooes"]=DF["set-fixchooes"].replace(/ /g,"");var b=DF["set-fixchooes"].split(","),c=0;for(i=0;i<a.length;i++)for(var d=0;d<b.length;d++)a[i]==b[d]&&c++;if(c==b.length){var e={};$.each(b,function(a,b){e[a]=$("#header").find('[df-fixed-id="'+b+'"]')});var f={};f[0]=$("#header").offset().top;var g=0,h=function(a,b,c){for(f[c+1]=e[c].outerHeight(),a.parent().height(a.outerHeight()),i=0;i<=c;i++)g+=f[i];0==a.hasClass("fixed")&&$(this).scrollTop()>a.parent().offset().top-g?(a.addClass("fixed"),a.css({top:g}),f[c+1]=e[c].outerHeight(),a.parent().height(a.outerHeight())):1==a.hasClass("fixed")&&$(this).scrollTop()<=a.parent().offset().top-g&&(a.removeClass("fixed"),a.parent().height("")),g=0};$.each(b,function(a,b){h(e[a],"scroll",a),$(document).on("scroll",function(){h(e[a],"scroll",a)}),$(window).on("resize",function(){h(e[a],"scroll",a)})})}};dffixed()}
	...
	// 게시판 카테고리 클래스 부여 2017-02-09
	$(".xans-board-listpackage").each(function(){var a=$(this),b=[];a.find("#board_category").find("option").each(function(){""!=$(this).val()&&(b[$(this).val()]=$(this).text())}),a.find(".category").each(function(){var a=$(this),c=a.text();for(k in b)c==b[k]&&a.wrapInner('<span class="catecode'+k+'"></span>')})});


	// sns 아이콘 사용하면 타이틀 보이기 2016-11-22
	$("#snslink").each(function(){var a=!1;$(this).find("li").each(function(){"none"!=$(this).css("display")&&(a=!0)}),1==a&&$(this).show()});


	// sns 로그인 사용하면 영역 보이기 2018-01-21
	$("#sns-login").each(function(){var a=!1;$(this).find("li").each(function(){"none"!=$(this).css("display")&&(a=!0)}),1==a&&$(this).show()});


	// 상세페이지 상품설명 좌우정렬 2017-02-13
	"undefined"!=typeof DF["set-detailarea"]&&$("#df-detail-area").each(function(){$(this).css("text-align",DF["set-detailarea"])});


	// 게시판 첨부파일 출력제어
	$("#boardWriteForm").length>0&&$("#boardWriteForm").find(".attachfile").not(".displaynone").each(function(){var g,a=$(this),b=2,c=1,d=a.find("tr").length,e=a.find("tr input:checkbox").length,f=function(b){a.find("tr .attachabtnArea").hide(),b.find(".attachabtnArea").show()};a.find("tr").each(function(a){str='<span class="attachabtnArea"> ',d>a+1&&(str+='<a href="#none" class="attachfile-add"><i class="fa fa-plus"></i></a> '),(0==e&&c<a+1||e>0&&e<a)&&(str+='<a href="#none" class="attachfile-remove"><i class="fa fa-minus"></i></a>'),str+=" </span>",$(this).find("td").append(str),$(this).hide(),g=e>0?e:b-1,g>=a&&($(this).show(),g==a&&f($(this)))}),a.find(".attachfile-add").click(function(){g=$(this).parent().parent().parent().index()+1,d<=g||(a.find("tr").eq(g).show(),f(a.find("tr").eq(g)))}),a.find(".attachfile-remove").click(function(){g=$(this).parent().parent().parent().index(),c>g||e>0&&e>=g||(a.find("tr").eq(g).hide(),f(a.find("tr").eq(g-1)))})});


	// 상품목록 베스트아이템 숫자 넣기 2016-11-22
	$(".xans-product-listrecommend.df-list-product").find(".label-best").each(function(a){$(this).append(' <span class="label-bestNum">'+(a+1)+"</span>")});


	// 상품상세페이지 상세이미지(추가이미지폼) 조절 2017-02-08
	$("#df-product-detail").each(function(){var a=$(this),b=a.find(".listImg").find("li"),c=a.find(".BigImage");$("<img/>").attr("src",c.attr("src")).load(function(){this.width<$(".imgArea-wrap").width()&&a.find(".keyImg").width(this.width),b.length>1?b.eq(0).find(".ThumbImage").attr("src",c.attr("src")):b.eq(0).hide(),a.find(".listImg").show()})});


	//상세페이지 이미지 이전, 다음상품 버튼 이벤트 2017-05-09
	$("#df-product-detail").each(function(){var a=$(this),b=a.find(".thumbnail"),c=$("#image_zoom_small");b.add(c).on({mouseenter:function(){b.addClass("on")},mouseleave:function(){b.removeClass("on")}})});


	// 상품상세페이지 탭이동 2017-02-07
	$("#df-product-detail, #df-detail-area").each(function(){$(this).find(".movetab").on("click",function(a){$("html, body").stop().animate({scrollTop:$($(this).attr("href")).offset().top-($("#header .bar").outerHeight()+$("#header #lnb").outerHeight()-1)},650,"easeInOutQuart"),a.preventDefault(),a.stopPropagation()})});


	//상품상세페이지 스마트추천 슬라이딩 2017-02-07
	$('#df-smartrecommend').find('.sr-list').each(function(){
		var _this = $(this);
		_this.children('ul').bxSlider({
			auto:true,
			autoHover:true,
			...
			nextText:'<i class="fa fa-angle-right" aria-hidden="true"></i>',
			onSliderLoad:function(){
				if(_this.find('.bx-pager-item').length<=1){
					_this.find('.bx-pager').hide();
				}
			}
		});
	});
	...
	// 상품상세페이지 구매옵션 픽스 2017-03-17
	void 0!=DF["set-detail-optionfix"]&&"on"==DF["set-detail-optionfix"]&&$("#df-product-detail").each(function(){function b(){0==a.hasClass("df-detail-fixed")&&$("#df-detail-area").offset().top<$(document).scrollTop()?(a.css("height",a.height()),a.addClass("df-detail-fixed")):1==a.hasClass("df-detail-fixed")&&$("#df-detail-area").offset().top>=$(document).scrollTop()&&(a.removeClass("df-detail-fixed"),a.css("height",""))}var a=$(this);1!=$.cookie("df-product-detail-option")&&void 0!=$.cookie("df-product-detail-option")&&a.addClass("close"),a.find(".infoArea").append('<a href="#none" class="df-detail-fixed-btn"><i>옵<br>션<br>보<br>기</i><span class="line1"></span><span class="line2"></span></a>'),a.find(".df-detail-fixed-btn").on("click",function(){a.toggleClass("close"),$.cookie("df-product-detail-option",1==a.hasClass("close")?"0":"1",{expires:1,path:"/",secure:!1})}),$(window).scroll(function(){b()}),b()});


	//상세페이지 관련상품(리스트,갤러리 타입설정) 2017-05-07
	$('#df-relation').each(function(){
		var settings = {
			auto:true,
			autoHover:true,
			autoDelay:5000,
		...
		var _this=$(this),$list=_this.find(".list"),$gallery=_this.find(".gallery");"list"!=DF["set-relation"]&&"all"!=DF["set-relation"]||$list.css({opacity:"1",visibility:"visible"}),"gallery"!=DF["set-relation"]&&"all"!=DF["set-relation"]||$gallery.css({opacity:"1",visibility:"visible"}).find(".prdList").bxSlider(settings),"list"==DF["set-relation"]?$gallery.remove():"gallery"==DF["set-relation"]&&$list.remove();
	});


	// 상품 상세페이지 상품후기&문의게시판 작성일,조회수 표시설정 2018-10-23
	var $detailBoard = $('#prdQnA, #prdReview');
	$detailBoard.each(function(){
		var $this = $(this);
		if($this.find('.hit-count.displaynone').length){
			$this.find('.hit-title').hide();
		}
		if($this.find('.date-count.displaynone').length){
			$this.find('.date-title').hide();
		}
	});

    
	// SNS 로그인 관련 2018-01-21
	var $outlogin = $('.sns-login');
	if($outlogin.length>0){
		if($outlogin.find('li').filter(function(){if($(this).css('display')!='none'){return true;}}).length<=0){
			$outlogin.hide();
		}
	}
	...
	// 탭 2017-09-25
	$(document).on('click', '.df-tab-cell', function(){
		var $this = $(this);
		$this.addClass('df-tab-selected').siblings('.df-tab-cell').removeClass('df-tab-selected');
		$($this.attr('df-data-id')).addClass('df-tab-content-selected').siblings('.df-tab-content').removeClass('df-tab-content-selected');
	});
	...
	// 기타 2018-09-10
	$("#footer .company-wrap .info").append('<p class="designed"> \x3c!--저작권자 표시인 DESIGNED BY DFLOOR 문구의 삭제 및 변경을 금지합니다. 무단복제·배포·부분사용을 금지하며 위반행위가 발견될 경우 유.무상의 접수가 불가능하며, 저작권법에 따라 법률적 조치가 이루어 질 수 있습니다.--\x3e HOSTING CAFE24<span class="line">|</span>DESIGNED By <a href="//dfloor.co.kr" target="_blank">DFLOOR</a></p>');


	// 블랙라벨(허가된사이트) 2019-04-14
	// $("#footer .company-wrap .info").append('<p class="designed"> \x3c!--저작권자 표시인 DESIGNED BY DFLOOR 문구의 삭제 및 변경을 금지합니다. 무단복제·배포·부분사용을 금지하며 위반행위가 발견될 경우 유.무상의 접수가 불가능하며, 저작권법에 따라 법률적 조치가 이루어 질 수 있습니다.--\x3e HOSTING CAFE24<span class="line">|</span>By <a href="//dfloor.co.kr" target="_blank">DFLOOR<i class="label">BLACK LABEL</i></a></p>');
